...
| --- | --- | --- | --- |
| Local agent, routine read and product operations | `aspect infra auth --persona=read` with browser/device login | `#platform-agent`: cluster read and port-forward; may derive 15-minute, product-scoped capabilities such as `guardian-mythra-observer` and `guardian-mythra-operator`; no Secrets or general writes | Unattended `offline_access` with a 30-day idle window; derived capabilities expire after 15 minutes |
...
| Codex Cloud | [`scripts/codex-cloud-setup.sh`](../scripts/codex-cloud-setup.sh) with Codex-scoped Cloudflare Access and portable `platform-agent` OIDC material | Same `#platform-agent` authority, including short-lived product capabilities through `aspect mythra ...`; no Secrets or general writes | OIDC cache plus Codex environment lifecycle; derived capabilities expire after 15 minutes |
| Cursor-hosted Cloud Agent | [`.cursor/environment.json`](../.cursor/environment.json) runs [`scripts/agent-cloud-setup.sh`](../scripts/agent-cloud-setup.sh) with Cursor-scoped Cloudflare Access and a JIT TokenRequest | `guardian-cloud-agent-cursor`: platform-read-equivalent cluster read and port-forward plus 15-minute Mythra observer/operator capabilities; no Secrets or general writes. `write-basic` remains available only through a fresh device approval | Bootstrap token: 10 minutes to 1 hour; derived capabilities: 15 minutes; write-basic: Keycloak session with no `offline_access` |
