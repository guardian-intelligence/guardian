...
- Confidential-computing customer claims and the hostile-worker validation
  gate are defined in
  [postflight-confidential-computing-security-validation.md](postflight-confidential-computing-security-validation.md).
...
- The bar: meet AWS Firecracker's software-isolation posture, minus hardware confidential computing. We do not yet have SEV-SNP-attested encrypted-memory guests (SEV is off in BIOS on the f4 boxes; the rs4 plane is unverified), so memory-in-use confidentiality against a malicious host/operator is an accepted gap until the selected SNP architecture passes the linked release gates and is enabled on the rs4 workload nodes (Phase 4). Until then the host is trusted; tenants are isolated from each other, not from us. This is runtime memory confidentiality, distinct from build/release attestation (which needs no TEE).
- VMM substrate: QEMU/KVM with `-cpu host` and the existing pinned `pc-q35-8.2` machine ABI. `microvm` and Cloud Hypervisor are rejected for the first confidential release so one launch-measurement and conformance tuple can stabilize; their reconsideration gate is recorded in the confidential-computing security policy. The jail and network model remain VMM-agnostic.
