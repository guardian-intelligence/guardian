...
Latency budget, measured: merge → Flux reconcile + kubelet ConfigMap
propagation ≈ 1–2 minutes; SSE fan-out → all connected clients re-evaluated
< 1 s. The flip is gated on the epoch event, which fires once for
everyone, so clients move together regardless of how long git → cluster
took.
