...
| `images.lock` closure attestation | Same `images-lock-sign` workflow, same push | `cosign attest-blob --new-bundle-format --type https://guardianintelligence.org/attestations/images-lock/v1` over a small flat predicate (`gitCommit`, `lockSha256`, `sectionCounts`, `totalEntries`) generated by `src/infrastructure/cmd/images_lock_attest`; proves which commit and section-shape the signed bytes claim to represent, additive to the plain signature above | `ghcr.io/guardian-intelligence/supply-chain:images.lock-<sha256>-attestation` (distinct tag suffix so it never races the plain signature push); full field list and rationale in `docs/images-lock-spec.md` ("Provenance") |
...
The closure attestation verifies offline the same way, against the same
trusted root (no second trust root introduced):

```sh
cosign verify-blob-attestation --bundle images.lock.attestation.sigbundle \
  --certificate-identity "<lock identity above>" \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
