...
lives in the self-init `initialize` block of the server config, inlined in the
OpenBao HelmRelease (`docs/secrets.md`): a new secret in an existing scope is a
routine PR; a structural change (new namespace or mount) is the reinit ceremony,
automated as `aspect infra openbao-reinit`
...
- No component can mutate OpenBao policy at runtime; the blast radius of a
  compromised workload stays inside its namespace scope. One deliberate runtime
  lane exists and is narrower than policy: the platform-agent mints scoped
  `secrets-writer` tokens through a TokenRequest grant behind a fail-closed VAP —
  token issuance, never policy mutation.
