...
of what Guardian releases — every public CLI's release channels. Promotions
bump a lane in the same commit as the matching channel pin. Both public CLIs'
nightly Kargo stages carry the extra `yaml-update`;
`TestReleaseManifestCoversReleaseChannels` holds every automated or reviewed
bump to the same rule, and
`TestPipeToRemoteBoxChannelsAndReleaseManifestMoveTogether` keeps Pipe to
Remote Box's channel grammar and lane pairing explicit. Per digest it
verifies the countersignature in
