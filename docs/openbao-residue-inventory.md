...
| Cloudflare ExternalDNS token | Reimportable custody residue | `DELETE_ME.env` only during bootstrap import; then OpenBao KV path `kv/guardian/guardian-mgmt/tenant-guardian/dns/external-dns` | DNS reconciliation stops until reimported | Importer must delete `DELETE_ME.env` after write/readback verification. |
