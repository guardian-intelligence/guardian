...
Status: current Guardian OpenBao cold-start and stateful recovery inventory.
...
| - | - | - | - | - |
| Static seal key and key id | Data-loss-critical | Offline custody, placed out of band on the three dedicated OpenBao nodes | Retained OpenBao raft data cannot be unsealed after restart or node rebuild | During rotation, keep the previous key until every member has completed seal migration and rewrap verification. |
...
This proves the Git-declared system can start from scratch using only
custody-held residue.
...
There is no OpenBao snapshot automation. The custody-held `transit/backup`
export and deterministic importer are the recovery path; a manual raft
snapshot is not a recovery prerequisite.
