...
| - | - | - | - |
| `cloudflare_external_dns_api_token` | ExternalDNS in `external-dns` | Guardian KMS OpenBao in `tenant-guardian-kms`, path `kv/guardian/guardian-mgmt/tenant-root/dns/external-dns`, projected by External Secrets Operator | Zone `guardianintelligence.org`: `Zone Read`, `DNS Read`, `DNS Write` |
| `cloudflare_dns_lb_provisioner_api_token` | OpenTofu root `src/infrastructure/clusters/ash/bootstrap/opentofu/dns` during edge bootstrap or recovery | Off-cluster break-glass or CI secret store; injected only into the apply environment | Account: `Load Balancing: Monitors and Pools Read`, `Load Balancing: Monitors and Pools Write`; Zone `guardianintelligence.org`: `Zone Read`, `Load Balancers Read`, `Load Balancers Write` |
| `cloudflare_r2_access_key_id` and `cloudflare_r2_secret_access_key` | OpenTofu S3-compatible backend for repo-owned state | Off-cluster break-glass or CI secret store; injected only into the apply environment as `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` | R2 `Object Read & Write`, scoped to the OpenTofu state bucket `guardian-vault` |
