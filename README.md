...
The company site, PrivateCut, and the WUM web app live in the vite-plus
workspace `src/products/viteplus-monorepo/`:

```sh
cd src/products/viteplus-monorepo
vp install
vp dev            # or: vp run <app>#dev
vp run ready      # pre-merge gate: lint + test + typecheck + build
```

Its `README.md` and `AGENTS.md` cover the workspace commands and the
reproducible Bazel image build.
