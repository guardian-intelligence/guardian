...
| `games/wake-up-mythra/sim/client/clock` (wasm, no_std) | WUM's client tick discipline | Acquiring → Locked (±2% slew) → FastForward (big deficits) → SnapshotRequired (beyond the ring). No floats, no host clocks — the host feeds it times and executes its step-count directives |
...
| `games/wake-up-mythra/sim/client` (wasm) | WUM's composed client module | render smoothing plus the intent verbs, over the session core. Implements the session-module ABI the host demands (`SESSION_EXPORTS`); never feeds back into world state. Framework candidates graduate back into `chunkies` when a second game proves them |
