...
describe("the world is corrected under the screen", () => {
  ...
  // viewer actually reads, `Core.view()`, with no special casing — and it
  ...
  function ownDog(r: Awaited<ReturnType<typeof rig>>, dog: bigint) {
  ...
  function roster(r: Awaited<ReturnType<typeof rig>>, flags = 0): Snap | null {
    const view = r.core.view();
    if (!view) return null;
    const dv = new DataView(view.viewBytes.buffer, view.viewBytes.byteOffset);
    const n = dv.getUint32(0, true);
    ...
    return {
      tick: view.tick,
      dogs,
      corrected: (flags & PumpFlag.corrected) !== 0,
      debt: r.core.glideDebtCells(),
    };
  }

  /** Walkable cells, for sending dogs somewhere. */
  function walkableNodes(r: Awaited<ReturnType<typeof rig>>): number[] {
    const t = r.core.view()!.terrain!;
    const out: number[] = [];
  ...
  function farTarget(r: Awaited<ReturnType<typeof rig>>, from: { x: number; y: number }) {
    const t = r.core.view()!.terrain!;
   ...
   */
  function parkAnswers(r: Awaited<ReturnType<typeof rig>>): () => void {
  ...
  async function fillThePark(r: Awaited<ReturnType<typeof rig>>): Promise<void> {
    ...
    const filled = await r.until(() => r.core.state.dogCount >= 4, 2000);
  ...
  it("carries every dog across a correction, and stops carrying them", async () => {
    ...
    // the world again.
    const dog = 0x9007n;
    const r = await rig({ role: "player", myDog: dog });
    await r.establish();
    ...
    r.core.moveTo(farTarget(r, standing).node);
    let corrections = 0;
    let underWay = -1;
    const snaps: Snap[] = [];
    for (let t = 0; t < 1600; t += FRAME_MS) {
      ...
      const flags = await frameStep(r, answer);
      if ((flags & PumpFlag.corrected) !== 0) corrections++;
      const s = roster(r, flags);
      if (!s) continue;
      snaps.push(s);
    ...
    expect(r.core.glideDebtCells(), "correction still on the presenter").toBe(0);
  });
  ...
  it("carries a boosted dog through a busy park", async () => {
    ...
    for (let t = 0; t < 3000; t += FRAME_MS) {
      ...
      if (t >= nextClick && clicks < 6) {
        r.core.moveTo(clicks % 2 === 0 ? there : here);
        clicks++;
        nextClick = t + 300;
      }
    ...
    expect(r.core.state.events - eventsBefore, "journal events applied").toBeGreaterThanOrEqual(
      clicks + joins * 2,
    );
    ...
    expectDebtSettles(snaps, "across a correction");
    expect(r.core.glideDebtCells(), "correction still on the presenter").toBe(0);
    expect(r.core.state.resyncs, "resyncs during the stretch").toBe(0);
  });
  ...
  it("hands a renderer the corrected world on the very next frame", async () => {
    ...
    const r = await rig({ role: "player", myDog: 0x9005n });
    await r.establish();
    await bringTheDogIn(r, 0x9005n);
    await r.run(400);

    r.authority.step(12);
    const before = r.core.view()!.tick;
    r.deliver([r.authority.snapshot()]);
    r.core.pump();

    expect(r.core.view()!.tick).toBeGreaterThanOrEqual(before);
  });
});
