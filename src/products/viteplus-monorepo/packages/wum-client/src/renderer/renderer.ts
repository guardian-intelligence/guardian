...
import type { WumGame } from "../game.ts";
import { decodeViewDog, Q16 } from "../projections/view.ts";
...
/** Enough of a dog to place it, in Q16.16 park cells. */
export type DogPos = {
  readonly id: bigint;
  readonly xq: number;
  readonly yq: number;
...
/** Camera/self diagnostics the renderer writes per frame for the surface to publish. */
export type RendererDiag = {
  myX: number;
  myY: number;
  camX: number;
  camY: number;
...
/** What the surface shows on the renderer's behalf, outside the canvas. */
export type RendererSurface = {
  readonly diag: RendererDiag;
  readonly showRecenter: (show: boolean) => void;
  readonly setRoster: (names: string[], total: number) => void;
  /** Once per drawn frame: what was drawn, for the feel counters. */
  readonly sample: (
    t: number,
    tick: number,
    phaseQ16: number,
...
export class WumRenderer {
  ...
  constructor(canvas: HTMLCanvasElement, game: WumGame, surface: RendererSurface) {
    ...
    function prerenderTerrain(planes: TerrainPlanes): void {
      ...
      deckLayer.width = pw;
      deckLayer.height = ph;
      const g = context2d(groundLayer);
      const d = context2d(deckLayer);
    ...
    this.followCamera = () => setCamFree(false);
    this.frame = (now) => {
      ...
      for (const dg of dogs) {
        if (dg.id === myDog) {
          mine = dg;
          surface.diag.myX = dg.xq;
          surface.diag.myY = dg.yq;
        }
      ...
      const camY = cam[1];
      lastCam = [camX, camY];
      surface.diag.camX = camX;
      surface.diag.camY = camY;
      surface.diag.camFree = camFree;
      ...
      for (const dg of dogs) if (dg.flags & 1) drawDog(dg, camX, camY, myDog);
      surface.setRoster(names, dogs.length);
      surface.sample(now, Number(view.tick), view.phaseQ16, mine, camX, camY, dogs);
    };
  }
}
