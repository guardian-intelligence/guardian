...
async function rewriteCjsRequireOnCompiled(nitro) {
  ...
  if (missed.size > 0) {
    ...
    console.warn(
      `[guardian:rewrite-cjs-require] ${missed.size} external __require call(s) unrewritten: ${summary}`,
    );
  }
  if (replaced > 0)
    console.log(
      `[guardian:rewrite-cjs-require] rewrote ${replaced} __require() call(s) across ${touchedFiles} chunk(s) under ${path.relative(nitro.options.rootDir, serverDir) || serverDir}`,
    );
}
