...
var observability_plugin_default = definePlugin((nitroApp) => {
	nitroApp.hooks.hook("request", (event) => {
		...
		if (!span) return;
		const correlationID = h3Event.req.headers.get("X-Guardian-Correlation-Id".toLowerCase()) ?? correlationIDFromContext(context["guardian_correlation_id"]);
		...
		if (clientIPSource !== "") span.setAttribute("guardian.request.client_ip_source", clientIPSource);
	...
	nitroApp.hooks.hook("response", (response, event) => {
		...
		const durationMs = typeof requestStartedAt === "bigint" ? Number((process.hrtime.bigint() - requestStartedAt) / 1000000n) : 0;
	...
	nitroApp.hooks.hook("error", (error, context) => {
		...
		const url = h3Event?.url;
		const correlationID = h3Event?.req.headers.get("X-Guardian-Correlation-Id".toLowerCase()) ?? correlationIDFromContext((h3Event?.context)?.["guardian_correlation_id"]);
