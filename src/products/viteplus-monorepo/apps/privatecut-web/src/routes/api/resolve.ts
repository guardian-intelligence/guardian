...
// client attaches to its privatecut.link_* analytics events — the join key
...
export const Route = createFileRoute("/api/resolve")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        await ready;
        const span = tracer().startSpan("privatecut.resolve", { kind: SpanKind.SERVER });
        try {
          const { response, code } = await resolve(request, span);
          span.setAttribute("privatecut.resolve.code", code);
