...
// silent -- the paths simply stay absolute and the digest stops being
// reproducible. The build-time guard below is what catches it.
const packageRoot = fileURLToPath(new URL(".", import.meta.url));

const stripRouteManifestPaths = {
  name: "privatecut:strip-route-manifest-paths",
...
export default defineConfig({
  build: {
    rollupOptions: {
      ...
      // The privatecut-web-image workflow enforces pin == built digest, which
