// Isolated module for the source-built Hauler binary. Hauler's dependency
// tree (helm, containerd, cosign, ~350 modules) must not participate in the
// root module's version resolution: it forces Kubernetes library bumps onto
// the production operator (observed: client-go v0.36 breaking
// controller-runtime v0.22). This go.mod feeds an isolated go_deps extension
// instance in MODULE.bazel; nothing in the root module imports from it.
module github.com/guardian-intelligence/guardian/src/tools/hauler

go 1.26.4

// Hauler's vanity import host serves no go-get meta for the /v2 path
// (hauler.dev/go/hauler/v2 404s), so resolve the module straight from the
// GitHub repo, whose go.mod declares the vanity path. Pinned past v2.0.1 to
...
replace hauler.dev/go/hauler/v2 => github.com/hauler-dev/hauler/v2 v2.0.2-0.20260716145139-2680e57f2e7f
...
// registry` depends on this exact distribution/v3 revision; without the
// mirror, a Bazel-built hauler would serve with different registry behavior
// than upstream's released binary.
...
require hauler.dev/go/hauler/v2 v2.0.1
