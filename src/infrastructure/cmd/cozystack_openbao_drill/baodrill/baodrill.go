// Package baodrill verifies a live OpenBao raft cluster over kubectl exec:
// every member initialized, unsealed, HA-enabled, running the StatefulSet's
// pinned version, and agreeing on one cluster ID. It backs both the
// standalone status drill and the reinit automation's post-reset check.
package baodrill
...
func printStatus(ctx context.Context, runner kubectlRunner, cfg Config, replicas int, expectedVersion string) error {
