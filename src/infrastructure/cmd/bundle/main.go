...
// carries a tag, its tag form. Refs come pre-validated by
// imageset.ParseLock, which shares the tag/digest normalization.
func haulerRefForms(ref string) (digestForm, tagForm string) {
	repo, digest, err := imageset.SplitRef(ref)
	if err != nil {
		// Unreachable for ParseLock output; fail loudly if a caller feeds
		// an unvalidated ref.
		panic(err)
	}
	if name, _, _ := strings.Cut(ref, "@"); name != repo {
		tagForm = name
	}
	digestForm = repo + "@" + digest
