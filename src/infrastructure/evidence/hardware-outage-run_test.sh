...
printf 'latitude:%s\n' "${action}" >>"${HARDWARE_OUTAGE_EVENT_LOG}"
case "${action}" in
  status)
...
printf 'capture:%s\n' "${phase}" >>"${HARDWARE_OUTAGE_EVENT_LOG}"
...
grep -Fqx "capture:outage-down-failed" "${event_log}"
awk '
  $0 == "latitude:power_off" {saw_off = NR}
  $0 == "capture:outage-down-failed" {saw_failed = NR}
  $0 == "latitude:power_on" {saw_on = NR}
  END {exit saw_off > 0 && saw_failed > saw_off && saw_on > saw_failed ? 0 : 1}
' "${event_log}"

grep -Fq "hardware outage step failed; capturing outage-down-failed" "${tmpdir}/stderr.log"
