...
func testOpenBaoOpenTofuBootstrap(t *testing.T) {
	...
	assertTextContains(t, mainTF, `from = vault_mount.kv`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `from = vault_policy.external_dns`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `from = vault_kubernetes_auth_backend_role.external_dns`, "openbao-root-bootstrap main.tf")
	if strings.Count(mainTF, "destroy = false") != 3 {
		t.Fatalf("openbao-root-bootstrap destroy=false removed blocks = %d, want 3", strings.Count(mainTF, "destroy = false"))
	}
	assertTextNotContains(t, mainTF, `resource "vault_mount" "kv"`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `resource "vault_policy" "external_dns"`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `resource "vault_kubernetes_auth_backend_role" "external_dns"`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `external_dns_secret`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `path "${local.kv_mount}/data/${local.external_dns_secret}"`, "openbao-root-bootstrap main.tf")
