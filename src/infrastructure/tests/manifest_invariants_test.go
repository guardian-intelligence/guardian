...
func testOpenBaoOpenTofuBootstrap(t *testing.T) {
	const versionsPath = "src/infrastructure/bootstrap/openbao-root-bootstrap/versions.tf"
	versionsBytes := readRunfile(t, versionsPath)
	versions := string(versionsBytes)
	...
	assertPartialS3Backend(t, versionsBytes, versionsPath, "opentofu/guardian-mgmt-openbao.tfstate", "openbao-root-bootstrap backend.s3")
	...
	assertTextContains(t, mainTF, `resource "vault_mount" "kv"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `type        = "kv-v2"`, "openbao-root-bootstrap main.tf")
	...
	assertTextContains(t, mainTF, `resource "vault_kubernetes_auth_backend_config" "guardian_mgmt"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `kubernetes_host        = "https://kubernetes.default.svc:443"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `disable_iss_validation = true`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `external_dns_secret   = "guardian/guardian-mgmt/tenant-guardian/dns/external-dns"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `resource "vault_policy" "external_dns"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `name = "guardian-external-dns"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `path "${local.kv_mount}/data/${local.external_dns_secret}"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `resource "vault_kubernetes_auth_backend_role" "external_dns"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `role_name                        = "guardian-external-dns"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `bound_service_account_names      = ["external-dns-secrets"]`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `bound_service_account_namespaces = ["external-dns"]`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `audience                         = "openbao"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `token_policies                   = [vault_policy.external_dns.name]`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `resource "vault_policy" "ops_controller"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `name = "guardian-openbao-ops-controller"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `path "sys/policies/acl/guardian-*"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `path "auth/${local.kubernetes_auth_mount}/role/guardian-*"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `path "sys/auth/${local.kubernetes_auth_mount}"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `path "sys/auth/${local.kubernetes_auth_mount}" {
      capabilities = ["create", "read", "update", "delete"]
    }`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `path "sys/auth/${local.kubernetes_auth_mount}/tune"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `path "sys/mounts/${local.kv_mount}"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `path "sys/mounts/${local.kv_mount}" {
      capabilities = ["create", "read", "update", "delete"]
    }`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `path "sys/mounts/${local.kv_mount}/tune"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `resource "vault_kubernetes_auth_backend_role" "ops_controller"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `role_name                        = "guardian-openbao-ops-controller"`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `bound_service_account_names      = ["openbao-ops-controller"]`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `bound_service_account_namespaces = ["tenant-guardian"]`, "openbao-root-bootstrap main.tf")
	assertTextContains(t, mainTF, `token_policies                   = [vault_policy.ops_controller.name]`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `resource "vault_policy" "secret_projection"`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `resource "vault_kubernetes_auth_backend_role" "secret_projection"`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `auth/token/lookup-self`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `path "*"`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `path "sys/*"`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, `path "auth/*"`, "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, "vault_kv_secret", "openbao-root-bootstrap main.tf")
	assertTextNotContains(t, mainTF, "vault_generic_secret", "openbao-root-bootstrap main.tf")
	...
	for _, forbidden := range []string{
		...
		assertTextNotContains(t, mainTF, forbidden, "openbao-root-bootstrap main.tf")
	}
}
