...
bazel build //src/postflight/guestd/cmd/guestd
listener="$(src/postflight/runner/build.sh)"
sudo env POOL=tank \
  GUESTD_BIN="$(bazel cquery --output=files //src/postflight/guestd/cmd/guestd)" \
...
sudo env \
  ...
  bazel test //src/postflight/hostd/vm:vm_test --test_output=streamed
```
