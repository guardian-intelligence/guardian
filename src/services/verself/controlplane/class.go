...
var errRunnerClassUnresolved = errors.New("github runner class is unresolved")
...
// further grammar. Unresolved is not an error condition for the pipeline:
// the job row is still persisted and the delivery processed; the job just
// never creates demand.
//
...
func runnerClassForLabels(labels []string, prefix string) (string, error) {
