...
install -m 0644 "$here/server/server.wasm" "$svc/server.wasm"
...
install -m 0644 "$here/server/server.wasm" "$prod/live.wasm"
...
install -m 0644 "$here/client/telemetry.gen.ts" "$ts/mythrad-client-core/src/telemetry.gen.ts"
