...
func TestMeteredEventNamesCounter(t *testing.T) {
	...
	visualFailureBefore := testutil.ToFloat64(eventsByName.WithLabelValues("local", "company.hero_visual_integrity_failed"))
	...
	req := connect.NewRequest(&analyticsv1.PublishRequest{
		...
		Events: []*analyticsv1.Event{
			{Name: "company.hero_visual_integrity_failed", Path: "/", PropsJson: `{"visual.failure":"layout-shift"}`},
	...
	if res.Msg.GetAccepted() != 4 {
		t.Fatalf("accepted = %d, want 4", res.Msg.GetAccepted())
	}
	waitRows(t, sink, 4)

	if d := testutil.ToFloat64(eventsByName.WithLabelValues("local", "company.hero_visual_integrity_failed")) - visualFailureBefore; d != 1 {
		t.Errorf("visual_integrity_failed delta = %v, want 1", d)
	}
