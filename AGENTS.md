...
* Cloudflare config has exactly three owners; drift between declared and live edge config is a defect. Workloads own only their origin HTTP contract (Cache-Control/ETag headers — Electric is the reference) and never hold edge credentials. The in-cluster external-dns controller owns DNS records, reconciled from Git-declared CRs with a DNS-only token. All zone-singleton policy (load balancers, cache rulesets, zone settings) is declared in `src/infrastructure/bootstrap/guardian-mgmt-dns/` and applied with the custody dns-lb-provisioner token — never edited in the dashboard. A dashboard change is either backported into that root or reverted by the next apply.
