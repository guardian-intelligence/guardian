...
| - | - | - | - | - | - |
| dev | vs-dev-w0 | 206.223.228.101 | sv_vAPXaMxKM5epz | dev.aisucks.app | cluster `guardian-dev`; PR-preview surface: converge any branch via `guardian up src/sites/dev/site.yaml` |
...
| OS | Talos Linux v1.13.4: immutable, API-only, no SSH. Machine config generated by `guardian up` from `site.yaml` (serial-selected install disk, static network, single-node patches). |
...
| Control | `guardian` CLI: `up`, `down --yes`, `config site`. talosctl v1.13.4 and kubectl v1.36.1 ride in runfiles; per-cluster state in `~/.local/state/guardian/<cluster>/`. |
